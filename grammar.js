	...
	rules: {
		...
		file_control_command: ($) =>
			seq(
				choice($._inc_name),
				$._ws_sep,
				alias(choice(/[._a-zA-Z0-9]+/, /"[._a-zA-Z0-9]+"/), $.file_name),
				//$._ws_end,
			),
		...
		_operation: ($) =>
			seq(
				choice($.implied, $.relative, $.immediate, $.absolute, $.indirect),
				//$._ws_end,
			),

		implied: ($) => seq(alias($._implied_opcode, $.opcode)),
		relative: ($) =>
			seq(alias($._relative_opcode, $.opcode), $._ws_sep, $._expr),
		...
		indirect: ($) =>
			prec.dynamic(
				...
				seq(
					...
					token.immediate("("),
					$._expr,
					choice(seq(")"), seq($._reg_x, ")"), seq(")", $._reg_y)),
				),
			),
